...
def call_api(client, model, system_prompt, user_prompt, max_tokens=8000, temperature=0.7, json_mode=False):
    """调用 OpenAI 兼容 API（支持 Claude 代理），返回 (文本, usage_dict)

    json_mode=True 时启用 response_format={"type": "json_object"}（需接口支持）。
    """
