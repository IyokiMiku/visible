...
def build_regenerate_question_prompt(question: dict[str, Any], issues: list[str], plan_context: dict[str, Any], spec_text: str = "") -> str:
    ...
    image_rule = ""
    if question.get("has_original_docx_images") or question.get("protected_original_docx_block"):
        image_rule = (
            "\n## 原 DOCX 图片保护要求\n"
            "本题原 DOCX 含图片/公式/OLE 等视觉对象，系统会在生成 Word 时从原文档复制这些对象。"
            "你只能修正文字字段，不得删除题干中的“如图/下图/图示”等引用，不得编造图片内容或输出图片字段。\n"
        )
