...
def _print_generation_status(paper_index: dict[int, PaperPlan], meta) -> None:
    """打印最终成品缺失状态，帮助用户判断哪些卷还需要处理。"""
    missing_numbers = _missing_output_numbers(meta, paper_index)
    generated_count = max(0, len(paper_index) - len(missing_numbers))

    if generated_count:
        print(f"\n已有最终成品：{generated_count} 卷")
    if missing_numbers:
        ranges = _compact_number_ranges(missing_numbers)
        print(f"最终成品缺失卷号：{ranges}（共 {len(missing_numbers)} 卷）")
    else:
        print("全部已有最终成品，无需再次生成。")
...
def print_preflight_dialog(result: PreflightResult, meta, paper_index: dict[int, PaperPlan]) -> None:
    ...
    print(f"最终成品缺失卷号：{missing_range}（共 {len(result.missing_sequence_numbers)} 卷）")
    print("\n检查项：")
...
def _repaired_docx_path_for_paper(paper) -> Path:
    manual_dir = manual_paper_dir_for_meta(paper.meta) if paper.meta else MANUAL_PAPER_DIR
    return manual_dir / f"{paper.paper_label}_repaired.docx"


def _point_questions_to_source_docx(questions: list[dict], source_path: Path) -> None:
    """Make final DOCX generation use the repaired DOCX as the authoritative layout source."""
    source_text = str(source_path)
    for question in questions or []:
        question["source_docx_path"] = source_text
        question["source_path"] = source_text


def _prefer_existing_repaired_docx(paper, loaded_paper) -> bool:
    """Reload an existing repaired DOCX so images/formulas keep the verified repaired layout."""
    repaired_path = _repaired_docx_path_for_paper(paper)
    current_path = Path(getattr(loaded_paper, "path", "") or "")
    if not repaired_path.exists() or current_path == repaired_path:
        return False
    try:
        repaired_paper = load_manual_paper(repaired_path, paper)
    except Exception as exc:
        print(f"  → 提示：发现修复版 DOCX 但加载失败，将继续使用当前来源：{exc}")
        return False
    loaded_paper.path = repaired_paper.path
    loaded_paper.title = repaired_paper.title
    loaded_paper.questions = repaired_paper.questions
    loaded_paper.warnings = repaired_paper.warnings
    _point_questions_to_source_docx(loaded_paper.questions, repaired_path)
    print(f"  已优先采用修复版 DOCX 作为版式来源：{repaired_path}")
    return True
...
def _repaired_to_manual_docx(paper, questions):
    """修复完成后，将题目回写为 DOCX，保留原题中的图片/公式对象。"""
    try:
        from docx import Document
        from docx.shared import Pt, RGBColor
        from 核心脚本.docx_utils1 import add_labeled_text, add_paragraph_with_style
        from .docx_generation import _append_original_question_block
        from .paper_assembler import (
            _heading_with_score_summary,
            _normalize_heading,
            _ordered_groups,
            _question_lines,
        )
        ...
        docx_path = manual_dir / f"{paper.paper_label}_repaired.docx"
        ...
        def add_text_paragraph(text, *, red=False, bold=False):
            text = str(text or "")
            if text.startswith(("【答案】", "【解析】", "【详解】", "【分析】")):
                label = text[:text.index("】") + 1]
                body = text[len(label):]
                return add_labeled_text(
                    doc,
                    "【解析】" if label in ("【详解】", "【分析】") else label,
                    body,
                    color=(255, 0, 0) if red else None,
                    bold_label=bold,
                )
            return add_paragraph_with_style(
                ...
                font_size=10.5,
                bold=bold,
                font_color=(255, 0, 0) if red else None,
                space_after=2,
            )

        display_no = 1
        prev_heading = ""
        for group_index, ((heading, qtype), items) in enumerate(_ordered_groups(questions).items()):
            section_heading = _normalize_heading(heading, qtype, group_index)
            if section_heading != prev_heading:
                if display_no > 1:
                    doc.add_paragraph("")
                add_text_paragraph(_heading_with_score_summary(section_heading, qtype, items, paper), bold=True)
                doc.add_paragraph("")
            prev_heading = section_heading

            for question in items:
                if question.get("protected_original_docx_block"):
                    copied = _append_original_question_block(doc, question)
                    if copied:
                        display_no += 1
                        continue
                    print(f"  → 警告：第{question.get('question_no')}题原始题块复制失败，回退为文本回写。")

                for line in _question_lines(question, display_no):
                    red = line.startswith(("【答案】", "【解析】", "【详解】", "【分析】"))
                    add_text_paragraph(line, red=red)
                doc.add_paragraph("")
                display_no += 1

        doc.save(str(docx_path))
        _point_questions_to_source_docx(questions, docx_path)
        legacy_txt = manual_dir / f"{paper.paper_label}_repaired.txt"
        if legacy_txt.exists():
            try:
                legacy_txt.unlink()
            except Exception:
                print(f"  → 提示：旧 TXT 修复文件未能删除，请手动移走：{legacy_txt}")
        print(f"  已回写修复结果到：{docx_path}")
    except Exception as e:
        print(f"  回写修复 DOCX 失败（不影响继续处理）：{e}")
...
def _repair_questions_with_qc(paper: PaperPlan, loaded_paper, args: argparse.Namespace, planned_total: int = 0, score_multiplier: int = 1) -> tuple[list[dict], list[dict], int, int]:
    ...
    if args.max_fix_rounds <= 0:
        ...
        if dist_fixed:
            _repaired_to_manual_docx(paper, loaded_paper.questions)
...
def _process_single_paper(
    ...
    try:
        ...
        if loaded_paper is None:
            ...
            _prefer_existing_repaired_docx(paper, loaded_paper)
            _update_run_record(
                paper,
                **{
                    "输入材料记录": {
    ...
    for result in results:
        ...
        try:
            ...
            _update_run_record(
                paper,
                最终结果={
                    "一句话说明": f"{paper.paper_label} 输出 DOCX 图片校验失败，本卷未交付。",
                    "状态": "处理失败，需要重新处理或人工排查",
                    "错误说明": result.error,
                },
            )
        ...
        if needs_manual:
            ...
            _cleanup_api_image_cache(paper, reason="最终文档已自动通过并交付")
...
def _parse_ai_shortfall_response(text: str) -> tuple[list[Any], str]:
    """解析补题响应，兼容 questions 对象、数组和单题对象。"""
    import re
    ...
    if not raw:
        return [], "AI 返回为空"
    if raw.startswith("```"):
        raw = re.sub(r"^```\w*\s*|```$", "", raw, flags=re.DOTALL).strip()

    def unwrap(value: Any) -> tuple[list[Any], str]:
        ...
        if isinstance(value, dict):
            ...
            if value.get("stem"):
                return [value], ""
            return [], "AI 返回 JSON 对象中没有 questions 数组"
        return [], f"AI 返回 JSON 不是题目数组，而是 {type(value).__name__}"
...
def _ai_fill_shortfall(
    ...
    remaining: dict[str, list[dict]] = {key: list(plans) for key, plans in shortfall.items() if plans}
    total_needed = sum(len(plans) for plans in remaining.values())
    ...
    system = (
        ...
        "题型、难度、数量不得偏离。只输出一个纯 JSON 对象，格式为 {\"questions\":[...]}，"
        "不要输出 Markdown、解释或代码块。"
    )

    def shortfall_lines(current: dict[str, list[dict]]) -> tuple[list[str], int]:
        lines = []
        count = 0
        for key, plans in current.items():
            if not plans:
                continue
            qtype, diff = key.split("|", 1)
            lines.append(f"- {qtype}：{len(plans)} 道（{diff}）")
            count += len(plans)
        return lines, count

    def build_prompt(current: dict[str, list[dict]], attempt_no: int) -> tuple[str, int]:
        lines, needed = shortfall_lines(current)
        retry_note = "" if attempt_no == 1 else f"\n注意：前面响应数量不足。本次只补剩余 {needed} 道，必须一次性补齐。\n"
        prompt = (
            f"请为以下考试生成 {needed} 道题目。{retry_note}\n"
            f"【课程】{paper.module}\n"
            ...
            f"2. 题型、难度、数量必须与上方清单完全一致\n"
            f"3. 选项应具有干扰性，正确选项不暴露\n"
            f"4. 每题必须包含完整题干、选项（单选题/多选题/判断题）、答案和解析\n"
            f"5. 判断题答案填\"A\"（正确）或\"B\"（错误）\n"
            f"6. 返回 JSON 对象，questions 数组长度必须等于 {needed}：\n"
            f'{{\n  "questions": [\n    {{\n      "stem": "题干文本",\n'
            f'      "options": ["A. 选项A", "B. 选项B", "C. 选项C", "D. 选项D"],\n'
            f'      "answer": "A",\n      "explanation": "解析文本",\n'
            f'      "question_type": "题型名称",\n      "difficulty": "简单/适中/困难"\n    }}\n  ]\n}}\n\n'
            f'{spec_text}'
        )
        return prompt, needed

    def accept_item(item: dict[str, Any], rejected: list[str], idx: int) -> bool:
        if not isinstance(item, dict):
            rejected.append(f"第{idx}项不是对象：{type(item).__name__}")
            return False
        if not item.get("stem"):
            rejected.append(f"第{idx}项缺少 stem 字段")
            return False

        item_qtype = (item.get("question_type") or item.get("_question_type") or "").strip()
        item_diff = (item.get("difficulty") or item.get("_target_difficulty") or "简单").strip()
        key = f"{item_qtype}|{item_diff}"
        if key not in remaining or not remaining[key]:
            rejected.append(f"第{idx}项题型/难度不在剩余缺口中：{item_qtype or '未填'}|{item_diff or '未填'}")
            return False

        remaining[key].pop()
        item["question_type"] = item_qtype
        item["difficulty"] = item_diff
        item["_ai_generated"] = True
        item["_question_type"] = item_qtype
        item["_target_difficulty"] = item_diff
        item["_course_id"] = course_id
        item["_kpoint_ids"] = kpoint_ids
        # 确保 options 是列表；保留 $...$ LaTeX，交给 DOCX 公式转换链路处理。
    ...
    trace_paths: list[Any] = []
    session_usage = _new_usage_summary(config=config)
    daily_usage = _load_daily_usage()
    max_rounds = int(config.get("ai_shortfall_max_rounds") or 4)

    for attempt_no in range(1, max_rounds + 1):
        _lines, needed_now = shortfall_lines(remaining)
        if needed_now <= 0:
            break
        if attempt_no > 1:
            print(f"    AI 补题数量不足，继续补剩余 {needed_now} 道（第 {attempt_no}/{max_rounds} 次）...")
        prompt, needed_now = build_prompt(remaining, attempt_no)
        text, usage = call_api(client, model, system, prompt, max_tokens=16000, temperature=temperature, json_mode=True)
        trace_name = "ai_shortfall_response.json" if attempt_no == 1 else f"ai_shortfall_response_retry{attempt_no}.json"
        trace_path = _write_api_trace_json(
            paper,
            trace_name,
            {
                "paper_label": paper.paper_label,
                "attempt": attempt_no,
                "needed_this_attempt": needed_now,
                "total_needed": total_needed,
                "remaining_before_call": {key: plans for key, plans in remaining.items() if plans},
                "raw_response": text,
                "usage": usage,
            },
        )
        ...
        data, parse_note = _parse_ai_shortfall_response(text)
        if parse_note:
            all_rejected.append(parse_note)
        before_count = len(result)
        rejected: list[str] = []
        for idx, item in enumerate(data or [], 1):
            if accept_item(item, rejected, idx):
                result.append(item)
        all_rejected.extend(rejected)
        accepted = len(result) - before_count
        if accepted:
            print(f"    本次 AI 补题接收 {accepted} / {needed_now} 道")
        else:
            preview = (text or "").strip().replace("\n", " ")[:500]
            reason = parse_note or ("；".join(rejected[:5]) if rejected else "未解析到含 stem 的题目对象")
            print(f"    本次 AI 补题未得到可用题：{reason}")
            if preview:
                print(f"    响应预览：{preview}")

    _print_token_summary(session_usage, daily_usage)

    remaining_count = sum(len(plans) for plans in remaining.values())
    if result:
        print(f"    AI 补题解析成功：{len(result)} / {total_needed} 道，原始响应：{trace_paths[-1] if trace_paths else ''}")
        if trace_paths and len(trace_paths) > 1:
            print(f"    AI 补题共调用 {len(trace_paths)} 次，首轮响应：{trace_paths[0]}")
    else:
        reason = "；".join(all_rejected[:5]) if all_rejected else "未解析到含 stem 的题目对象"
        print(f"    AI 补题未得到可用题：{reason}")
        if trace_paths:
            print(f"    原始响应已保存：{trace_paths[-1]}")
    if remaining_count:
        lines, _needed = shortfall_lines(remaining)
        print(f"    AI 补题仍缺 {remaining_count} 道：{'；'.join(lines)}")
    if all_rejected and result:
        print(f"    AI 补题有 {len(all_rejected)} 项被丢弃/忽略：{'；'.join(all_rejected[:3])}")
...
def _image_cache_dir(paper: PaperPlan) -> Path:
    cache_dir = _api_trace_dir(paper) / "images"
    cache_dir.mkdir(parents=True, exist_ok=True)
    return cache_dir


def _image_manifest_path(paper: PaperPlan) -> Path:
    return _image_cache_dir(paper) / "images_manifest.json"


def _load_image_manifest(paper: PaperPlan) -> dict[str, Any]:
    path = _image_manifest_path(paper)
    if not path.exists():
        return {"images": []}
    try:
        data = json.loads(path.read_text(encoding="utf-8"))
...
def _save_image_manifest(paper: PaperPlan, manifest: dict[str, Any]) -> None:
    _image_manifest_path(paper).write_text(json.dumps(manifest, ensure_ascii=False, indent=2, default=str), encoding="utf-8")


def _cleanup_api_image_cache(paper: PaperPlan, *, reason: str = "") -> None:
    """Remove cached API images after a final DOCX has been accepted by the flow."""
    cache_dir = _api_trace_dir(paper) / "images"
    ...
    try:
        ...
        print(f"  已清理本卷 API 图片缓存{suffix}：{cache_dir}")
    except Exception as exc:
        print(f"  → 提示：API 图片缓存未能自动删除，请手动清理：{cache_dir}（{exc}）")
...
def _image_extension(url: str, content_type: str = "") -> str:
    ...
    return guessed if guessed in {".jpg", ".jpeg", ".png", ".gif", ".bmp", ".webp"} else ".png"


def _download_image(url: str, cache_dir: Path | None = None, *, cookie: str = "") -> dict[str, Any]:
    """Download an API image into the paper cache and return local metadata."""
    url = str(url or "").strip()
    if not url:
        return {"download_status": "missing_url", "download_error": "empty url"}
    if not re.match(r"^https?://", url, flags=re.I):
        local = Path(url)
        if local.exists():
            data = local.read_bytes()
            return {"download_status": "local", "local_path": str(local), "sha256": hashlib.sha256(data).hexdigest(), "byte_size": len(data)}
        return {"download_status": "unsupported_url", "download_error": f"not http/local: {url[:80]}"}
    if cache_dir is None:
        cache_dir = API_RAW_OUTPUT_DIR / "图片缓存"
    cache_dir.mkdir(parents=True, exist_ok=True)
    url_hash = hashlib.sha256(url.encode("utf-8")).hexdigest()
    existing = sorted(cache_dir.glob(f"{url_hash[:16]}_*"))
    if existing:
        data = existing[0].read_bytes()
        return {"download_status": "cached", "local_path": str(existing[0]), "sha256": hashlib.sha256(data).hexdigest(), "byte_size": len(data), "url_hash": url_hash}
    try:
        import urllib.request
        request = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0", "Referer": "https://yanyi.zxxk.com/", **({"Cookie": cookie} if cookie else {})})
        with urllib.request.urlopen(request, timeout=20) as response:
            data = response.read()
            content_type = response.headers.get("Content-Type", "")
        if not data:
            return {"download_status": "failed", "download_error": "empty response", "url_hash": url_hash}
        sha = hashlib.sha256(data).hexdigest()
        path = cache_dir / f"{url_hash[:16]}_{sha[:16]}{_image_extension(url, content_type)}"
        if not path.exists():
            path.write_bytes(data)
        return {"download_status": "downloaded", "local_path": str(path), "sha256": sha, "byte_size": len(data), "content_type": content_type, "url_hash": url_hash}
    except Exception as exc:
        return {"download_status": "failed", "download_error": str(exc), "url_hash": url_hash}
...
_PULLED_DOCX_RESIDUAL_FORMULA_RE = re.compile(
    r"(\$[^$]+\$|\\\(|\\\)|\{\{?math:|\\(?:frac|sqrt|times|cdot|div|leq?|geq?|neq|ne|approx|Omega|rho|Phi|phi|Delta|mu|pi|left|right|mathrm|text)\b|[A-Za-z0-9][_^]\{[^{}]+\})"
)
_PULLED_DOCX_FORBIDDEN_MATH_CHARS = set(
    "₀₁₂₃₄₅₆₇₈₉⁰¹²³⁴⁵⁶⁷⁸⁹×÷√≤≥≠αβγδεζηθικλμνξοπρστυφχψω"
    "ΑΒΓΔΕΖΗΘΙΚΛΜΝΞΟΠΡΣΤΥΦΧΨΩ"
)
_PULLED_DOCX_UNIT_CHARS = {"Ω"}
_EMU_PER_CM = 360000
_PULLED_DOCX_MAX_IMAGE_W_CM = 12.0
_PULLED_DOCX_MAX_IMAGE_H_CM = 8.0


def _iter_docx_visible_text(doc):
    ...
    for table in doc.tables:
        for row in table.rows:
            for cell in row.cells:
                yield from _iter_docx_visible_text(cell)


def _text_has_formula_source(text: str) -> bool:
    if not text:
        return False
    return bool(_PULLED_DOCX_RESIDUAL_FORMULA_RE.search(text)) or any(
        ch in (_PULLED_DOCX_FORBIDDEN_MATH_CHARS - _PULLED_DOCX_UNIT_CHARS) for ch in text
    )


def _forbidden_math_chars_in_text(text: str) -> list[str]:
    found = set()
    for idx, ch in enumerate(text or ""):
        if ch not in _PULLED_DOCX_FORBIDDEN_MATH_CHARS:
            continue
        if ch == "Ω":
            left = text[:idx].rstrip()
            right = text[idx + 1:idx + 2]
            prev_ch = left[-1] if left else ""
            if (prev_ch.isdigit() or prev_ch == ")") and not (right.isalpha() or right.isdigit()):
                continue
        found.add(ch)
    return sorted(found)


def _docx_image_count(docx_path: Path) -> int:
    import zipfile
    with zipfile.ZipFile(docx_path) as zf:
        return sum(1 for name in zf.namelist() if name.startswith("word/media/") and not name.endswith("/"))


def _validate_pulled_docx_layout(docx_path: Path, source_texts: list[str] | None = None, expected_images: int = 0) -> None:
    from docx import Document
    import zipfile
    issues: list[str] = []
    doc = Document(str(docx_path))
    placeholder_count = 0
    for text in _iter_docx_visible_text(doc):
        placeholder_count += text.count("[图片]") + text.count("【图片】")
        if _PULLED_DOCX_RESIDUAL_FORMULA_RE.search(text):
            issues.append(f"残留 LaTeX/公式原码：{text[:80]}")
        forbidden = _forbidden_math_chars_in_text(text)
        if forbidden:
            issues.append(f"残留 Unicode 数学字符 {''.join(forbidden)}：{text[:80]}")
    if placeholder_count:
        issues.append(f"仍有图片占位符 {placeholder_count} 处")
    source_needs_omml = any(_text_has_formula_source(text) for text in (source_texts or []))
    has_omml = False
    with zipfile.ZipFile(docx_path) as zf:
        for name in zf.namelist():
            ...
            if "<m:oMath" in xml or "<m:oMathPara" in xml:
                has_omml = True
            for m in re.finditer(r"<wp:extent[^>]*\bcx=\"(\d+)\"[^>]*\bcy=\"(\d+)\"", xml):
                w_cm = int(m.group(1)) / _EMU_PER_CM
                h_cm = int(m.group(2)) / _EMU_PER_CM
                if w_cm > _PULLED_DOCX_MAX_IMAGE_W_CM + 0.05 or h_cm > _PULLED_DOCX_MAX_IMAGE_H_CM + 0.05:
                    issues.append(f"图片显示尺寸过大：{w_cm:.1f}cm x {h_cm:.1f}cm（{name}）")
    actual_images = _docx_image_count(docx_path)
    if expected_images and actual_images < expected_images:
        issues.append(f"源题包含 {expected_images} 张图片，但 DOCX 仅嵌入 {actual_images} 张图片")
    if source_needs_omml and not has_omml:
        issues.append("源题包含公式/数学字符，但生成 DOCX 中未发现 <m:oMath>。")
    if issues:
        preview = "\n  - ".join(issues[:12])
        extra = "" if len(issues) <= 12 else f"\n  - ... 共 {len(issues)} 项问题"
        raise ValueError(f"API 拉题 DOCX 排版校验失败：\n  - {preview}{extra}")


def _localize_image_items(image_items, *, paper: PaperPlan | None = None, question_id: Any = "", field: str = "", cookie: str = ""):
    cache_dir = _image_cache_dir(paper) if paper else None
    localized = []
    for idx, item in enumerate(image_items or []):
        if not isinstance(item, dict):
            localized.append(item)
            continue
        img = dict(item)
        if not img.get("local_path"):
            img.update(_download_image(img.get("url") or img.get("src") or "", cache_dir, cookie=cookie))
        img.setdefault("question_id", question_id)
        img.setdefault("field", field)
        img.setdefault("image_index", idx)
        localized.append(img)
    return localized


def _localize_option_images(options, *, paper: PaperPlan | None = None, question_id: Any = "", cookie: str = ""):
    localized = []
    for opt in options or []:
        if not isinstance(opt, dict):
            ...
            continue
        new_opt = dict(opt)
        label = str(new_opt.get("label") or "").strip()
        new_opt["images"] = _localize_image_items(
            new_opt.get("images") or [],
            paper=paper,
            question_id=question_id,
            field=f"option_{label}" if label else "option",
            cookie=cookie,
        )
        localized.append(new_opt)
    return localized


def _image_download_records(question: dict) -> list[dict[str, Any]]:
    records = []
    for img in question.get("stem_images") or []:
...
def _save_pulled_questions_as_docx(paper: PaperPlan, questions: list[dict], score_multiplier: int = 1) -> Path | None:
    ...
    from docx.shared import Pt
    from 核心脚本.docx_utils1 import add_labeled_text, add_paragraph_with_style, add_structured_choice_question
    from 学科网API拉题移植版.html_content_converter import (
        convert_stem_html, convert_answer_html, convert_explanation_html,
    )
    from 学科网API拉题移植版.query_questions import DEFAULT_COOKIE
    ...
    if score_multiplier > 1:
        ...
        run = note.add_run(f"【注意】本卷题量减半，每题分值乘以{score_multiplier}，总分仍为100分")
        run.font.size = Pt(9)
    ...
    qno = 0
    heading_idx = 0
    source_texts_for_qc: list[str] = []
    image_manifest = _load_image_manifest(paper)
    image_records_by_key = {str(item.get("url") or item.get("url_hash") or item.get("local_path")): item for item in image_manifest.get("images", []) if isinstance(item, dict)}
    for qtype in type_order:
        ...
        for q in typed_questions:
            ...
            source_texts_for_qc.append(stem)
            question_id = q.get("questionId") or q.get("question_id") or q.get("_question_no") or qno
            localized_stem_images = _localize_image_items(
                stem_images,
                paper=paper,
                question_id=question_id,
                field="stem",
            ...
            localized_options = _localize_option_images(
                parsed_options or [],
                paper=paper,
                question_id=question_id,
                cookie=DEFAULT_COOKIE,
            )
            q["stem_images"] = localized_stem_images
            if localized_options:
                q["options"] = localized_options
            cache_records = _image_download_records(q)
            q["_api_image_cache"] = cache_records
            for record in cache_records:
                key = str(record.get("url") or record.get("url_hash") or record.get("local_path"))
                if key:
                    image_records_by_key[key] = record

            # 选项：优先使用结构化组件，统一处理公式和图片缩放。
            if localized_options or localized_stem_images:
                for opt in localized_options:
                    if isinstance(opt, dict):
                        source_texts_for_qc.append(str(opt.get("text") or ""))
                add_structured_choice_question(
                    doc,
                    f"{qno}．{stem}",
                    localized_options,
                    stem_images=localized_stem_images,
                    font_name="宋体",
                    font_size=10.5,
                )
            else:
                add_paragraph_with_style(doc, f"{qno}．{stem}", font_name="宋体", font_size=10.5, space_after=0)
                ...
                if isinstance(options, list):
                    for opt in options:
                        ...
                        if opt_text:
                            source_texts_for_qc.append(opt_text)
                            add_paragraph_with_style(doc, opt_text, font_name="宋体", font_size=10.5, space_after=0)
            ...
            source_texts_for_qc.extend([answer, analysis])
            # 回写转换后的值，供后续 paper_loader 拆题使用
            q["answer"] = answer
            q["analysis"] = analysis
            q["stem"] = stem
            if answer:
                add_labeled_text(doc, "【答案】", answer, content_font="宋体", content_size=10.5, bold_label=False)
            if analysis:
                add_labeled_text(doc, "【详解】", analysis, content_font="宋体", content_size=10.5, bold_label=False)

            doc.add_paragraph("")

    image_manifest["images"] = list(image_records_by_key.values())
    image_manifest["paper_label"] = paper.paper_label
    image_manifest["updated_at"] = time.strftime("%Y-%m-%d %H:%M:%S")
    _save_image_manifest(paper, image_manifest)
    ...
    expected_images = sum(
        1
        for record in image_records_by_key.values()
        if isinstance(record, dict) and record.get("local_path") and record.get("download_status") in {"downloaded", "cached", "local"}
    )
    _validate_pulled_docx_layout(output_path, source_texts_for_qc, expected_images=expected_images)
    return output_path
