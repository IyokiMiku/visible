...
def _is_formula_variable_text(text: str) -> bool:
    stripped = str(text or "").strip()
    return bool(stripped) and any(ch.isalpha() or ch.isdigit() for ch in stripped) and not all(ch in SYMBOL_CHARS for ch in stripped)


def _split_formula_run_by_italic(math_run) -> list:
    text_nodes = list(math_run.iter(qn("m:t")))
    if len(text_nodes) != 1:
        return [math_run]
    ...
    for ch in text:
        italic = _is_formula_variable_text(ch)
        if chunks and chunks[-1][0] == italic:
            chunks[-1] = (italic, chunks[-1][1] + ch)
        else:
            chunks.append((italic, ch))
    if len(chunks) <= 1:
        return [math_run]
    ...
    for idx, (_, chunk) in enumerate(chunks):
        current = math_run if idx == 0 else copy.deepcopy(math_run)
        current.find(qn("m:t")).text = chunk
        if idx > 0:
            anchor.addnext(current)
            anchor = current
        new_runs.append(current)
    return new_runs


def _set_paragraph_math(paragraph, *, make_red: bool) -> bool:
    """格式化段落内 Word 原生公式：变量字母/数字斜体，答案解析区再标红。"""
    changed = False
    for math_run in list(paragraph._p.iter(qn("m:r"))):
        for split_run in _split_formula_run_by_italic(math_run):
            text = "".join(node.text or "" for node in split_run.iter(qn("m:t")))
            r_pr = split_run.find(qn("w:rPr"))
            if r_pr is None:
                r_pr = OxmlElement("w:rPr")
                split_run.insert(0, r_pr)
            if make_red:
                _set_rpr_red(r_pr)
            if _is_formula_variable_text(text):
                _set_rpr_italic(r_pr)
            changed = True
    return changed
    ...
    for paragraph in paragraphs:
        ...
        if has_label:
            if _mark_from_label(paragraph):
                ...
                _set_paragraph_math(paragraph, make_red=True)
                changed += 1
            in_answer_area = True
            continue
...
def _mark_all_math_italic(paragraphs) -> int:
    changed = 0
    for paragraph in paragraphs:
        if _set_paragraph_math(paragraph, make_red=False):
            changed += 1
    return changed
...
def mark_docx(path: Path) -> int:
    ...
    changed += _mark_all_math_italic(doc.paragraphs)
    changed += _mark_all_math_italic(list(_iter_table_paragraphs(doc.tables)))
