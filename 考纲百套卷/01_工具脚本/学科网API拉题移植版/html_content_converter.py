...
def _parse_css_px(style_text, name):
    m = re.search(rf'{name}\s*:\s*([0-9.]+)\s*px', style_text or '', re.IGNORECASE)
    if not m:
        return 0
    try:
...
def _parse_img_tag(img_html):
    ...
    style_m = re.search(r'style\s*=\s*"([^"]*)"', img_html, re.IGNORECASE)
    style = style_m.group(1) if style_m else ''
    w = re.search(r'\bwidth\s*=\s*"?([0-9.]+)', img_html, re.IGNORECASE)
    h = re.search(r'\bheight\s*=\s*"?([0-9.]+)', img_html, re.IGNORECASE)
    display_w = int(float(w.group(1))) if w else _parse_css_px(style, 'width')
    display_h = int(float(h.group(1))) if h else _parse_css_px(style, 'height')
    if display_w > 0:
        item["display_w"] = display_w
    if display_h > 0:
        item["display_h"] = display_h
    return item
