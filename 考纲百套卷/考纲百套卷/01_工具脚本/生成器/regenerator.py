...
def fix_answer_distribution(questions: list[dict[str, Any]], max_swaps: int = 5) -> bool:
    ...
    for _ in range(max_swaps):
        ...
        for i, q in enumerate(questions):
            ...
            if qt == "单项选择题" and len(ans) == 1 and ans in "ABCD":
...
def normalize_regenerated_question(original: dict[str, Any], repaired: dict[str, Any]) -> dict[str, Any]:
    ...
    question["source_path"] = original.get("source_path", "")
    question["status"] = "repaired"
    question["issues"] = []
    question["issue_details"] = []
    question["fix_type"] = repaired.get("fix_type") or "auto"
...
def regenerate_question(
    ...
    if question.get("protected_original_docx_block"):
        return RegenerationResult(question, question, issues, "skipped", "题目含图片/公式对象/OLE 对象，保留原 DOCX 题块，跳过自动重生成")
