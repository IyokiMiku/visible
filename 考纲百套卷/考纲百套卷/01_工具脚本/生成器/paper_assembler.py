...
def _question_lines(question: dict[str, Any], display_no: int) -> list[str]:
    stem = _clean_text(question.get("stem")) or _clean_text(question.get("raw_text"))
    lines = [f"{display_no}. {stem}".rstrip()]
    lines.extend(_option_lines(question.get("options")))
    lines.append(f"【答案】{_clean_text(question.get('answer'))}")
    lines.append(f"【解析】{_analysis_text(question.get('analysis'))}")
    return lines
