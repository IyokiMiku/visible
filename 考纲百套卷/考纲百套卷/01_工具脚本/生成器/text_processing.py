...
def _normalize_generated_text(text):
    ...
    # 用深度感知扫描替代简单 .*? 正则：LaTeX 花括号如 X_{L甲} 内的 } 不应被误判为标记结束。
    math_chunks = []

    def _depth_aware_stash(source: str) -> str:
        result = []
        pos = 0
        length = len(source)
        while pos < length:
            start = source.find('{{math:', pos)
            if start == -1:
                result.append(source[pos:])
                break
            result.append(source[pos:start])
            content_start = start + 7  # len('{{math:')
            i = content_start
            depth = 0
            end = -1
            while i < length:
            ...
            index = len(math_chunks)
            math_chunks.append(chunk)
            result.append(f"@@MATH_MARKER_{index}@@")
            pos = end + 2
        return ''.join(result)

    text = _depth_aware_stash(text)
...
def _clean_paper_text(text):
    ...
    for line in lines:
        ...
        # 跳过 markdown 标题行（如 # 重庆市考纲百套卷、## 《xxx》）
        if re.match(r"^#{1,6}\s", stripped):
            ...
            # 其他 markdown 标题（如 # 重庆市考纲百套卷）直接丢弃
            continue
