...
def _repair_qc_issues_targeted(client, config, meta, topic, set_idx, spec_text,
    ...
    # 经过修复轮次后，仍有"略/无/—"等无意义解析占位的，直接去掉该行
    paper_text = _remove_stub_analysis(paper_text)

    return paper_text, severe, warnings, score, infos, repaired_rounds

def _remove_stub_analysis(paper_text: str) -> str:
    """去除【解析】略、【解析】无、【解析】— 等无意义占位行。"""
    return re.sub(r"【解析】\s*(略|无|—|－－|--)\s*[。.]?\s*(\n|$)", r"\n", paper_text)
