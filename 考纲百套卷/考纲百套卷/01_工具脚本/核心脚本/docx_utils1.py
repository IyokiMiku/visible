...
def _image_size_px(image_item):
    """读取图片像素尺寸；优先使用元数据，Pillow 不可用时安全降级。"""
    if isinstance(image_item, dict):
        width = _numeric_dim(image_item.get('width') or image_item.get('display_w'))
        height = _numeric_dim(image_item.get('height') or image_item.get('display_h'))
        if width and height:
            return width, height
...
def _fit_image_size_cm(width_px, height_px, max_w_cm, max_h_cm):
    """按最大宽高等比缩小图片；尺寸未知时使用最大框兜底。"""
    if width_px <= 0 or height_px <= 0:
    ...
    scale = 1.0
    if max_w_cm and width_cm * scale > max_w_cm:
        scale = max_w_cm / width_cm
...
def _add_image_run(paragraph, image_item, max_w_cm, max_h_cm, warn_label='图片'):
    ...
    if not path or not path.exists():
        ...
        return False
    width_px, height_px = _image_size_px(image_item)
    width_cm, height_cm = _fit_image_size_cm(width_px, height_px, max_w_cm, max_h_cm)
    try:
