...
def check_question_structured(
    ...
    has_answer_images = bool(
        isinstance(question.get("image_refs"), dict)
        and (question["image_refs"].get("answer") or question["image_refs"].get("options"))
    )
    skip_analysis_checks = qtype in SUBJECTIVE_TYPES and (len(answer) >= 10 or has_answer_images)
